...
def create_client_certificates(client_id, client_password, ca_password):
    client_private_key_filename = "./sign_keys_pair/clients/client" + str(client_id) + "-private-key.pem"
    client_private_key = generate_private_key(
        client_private_key_filename, client_password
    )
    print("Client private key: " + str(client_private_key))


    client_csr_filename = "./cert_requests/clients/client" + str(client_id) + "-csr.pem"
    generate_csr(
        client_private_key,
        filename=client_csr_filename,
        country="PT",
        state="Porto",
        locality="Porto",
        org="CLIENT_GRUPO_14",
    ...
    csr_file = open(client_csr_filename, "rb")
    csr = x509.load_pem_x509_csr(
        csr_file.read(), default_backend()
    )
    print("Client" + str(client_id) + " csr: " + str(csr))
    ...
    print("CA public-key: " + str(ca_public_key))
    ...
    print("CA private-key: " + str(ca_private_key))

    client_public_key_filename = "./sign_keys_pair/clients/client" + str(client_id) + "-public-key.pem"
    client_public_key = sign_csr(csr, ca_public_key, ca_private_key, client_public_key_filename)
